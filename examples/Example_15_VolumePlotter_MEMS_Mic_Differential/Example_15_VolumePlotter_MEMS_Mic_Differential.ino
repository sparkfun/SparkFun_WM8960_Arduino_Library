/******************************************************************************
  Example_15_VolumePlotter_MEMS_Mic_Differential.ino
  This example is very similar to example 11, however it uses a MEMS mic with
  differential signal as the source of the sound. Here we are using just one of
  VM2020 Breakouts plugged into the left channel.

  Note, with this VM2020 differential mic signal, we are going to add much more
  gain than in previous examples. We are going to use a MICBOOST of 29dB (max),
  and set the PGA to +8.25dB.
  
  Demonstrates reading I2S audio from the ADC, and plotting the audio samples on 
  the Arduino Serial Plotter.
  
  This example sets up differential analog audio input (on LINPUT1/LINPUT2), 
  ADC enabled as I2S peripheral, sets volume control, and Headphone output on 
  the WM8960 Codec.

  A MEMS mic should be connected to the left channel INPUT1 and INPUT2, they are
  labeled "LIN1" and "LIN2" on the board. These will provide the input 
  connections for the positive and negative signals from the MEMS mic.
  ...
  **********************
  ...
  Do you like this library? Help support SparkFun. Buy a board!
    ...
    https://www.sparkfun.com/products/21537
